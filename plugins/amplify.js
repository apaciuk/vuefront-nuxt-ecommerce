...
module.exports = (html, url) => {
  ...
  if(isAMP){
    ...
    html = html.replace('</head>',  `<link rel="canonical" href="/${url.replace('/amp', '')}"></head>`)

  } else {
    html = html.replace('</head>',  `<link rel="amphtml" hid="amphtml" href="/amp${url === '/' ? '' : url}"></head>`)
  }
  return html
}
